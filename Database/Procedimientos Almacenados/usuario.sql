...
BEGIN
    UPDATE infinite_knowledge.usuario SET usuario.dni=dni_,usuario.correo_electronico = correo_electronico_,usuario.contrasenia = contrasenia_,usuario.rol = rol_ WHERE usuario.usuario_id = usuario_id_;
END //
DELIMITER ;
...
BEGIN
    SELECT usuario.usuario_id, usuario.dni, usuario.correo_electronico, usuario.contrasenia, usuario.rol FROM infinite_knowledge.usuario WHERE usuario.rol = rol_ ORDER BY usuario.usuario_id ASC;
END //
DELIMITER ;
...
BEGIN
    SELECT usuario.usuario_id, usuario.dni, usuario.rol FROM infinite_knowledge.usuario WHERE usuario.correo_electronico = correo_electronico_ AND usuario.contrasenia = contrasenia_;
END //
DELIMITER ;
